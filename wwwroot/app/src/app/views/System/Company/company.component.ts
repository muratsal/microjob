...
export class CompanyComponent implements OnInit {
    ...
    getData(isExport: boolean) {
        ...
        if (isExport) {
            ...
            columnInfo = this.getDataConf().filter(x => x.isExport == true)
                .map((x) => ({
                    prop: x.prop,
                    localText: this.translate.instant('COMPANY.' + x.translate)
                }));

        }


        this.companyService.getData(this.filter, this.queryInfo, columnInfo, isExport)
    ...
    newItemDialog() {
        ...
        let emptyData = new CompanyData();

        emptyData.companyId = 0;
emptyData.companyName = '';
emptyData.logo = '';
emptyData.address = '';
emptyData.phone = '';
emptyData.email = '';
emptyData.website = '';
emptyData.taxAuthority = '';
emptyData.taxNumber = '';
...
emptyData.updatedUser = 0;
    ...
    editItemDialog(row: CompanyData) {
        this.openDialog(row, false);
    }

    openDialog(row: CompanyData, isNew: boolean) {
        ...
        this.dialog.open(ComfirmDialogComponent, {
            ...
            .subscribe(response => {
                if (response.comfirm == true) {
                    this.companyService.delete(deleteRow).subscribe((response: any) => {
                         this.navigationService.sessionControl(response);
                        if (response.isSuccess) {
