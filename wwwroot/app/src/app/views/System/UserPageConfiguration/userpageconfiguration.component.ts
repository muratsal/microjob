...
import { UserPageConfigurationDialogComponent } from './userpageconfiguration-dialog.component';
...
@Component({
    selector: 'userpageconfiguration-table',
    templateUrl: './userpageconfiguration.component.html',
    styleUrls: ['./userpageconfiguration.component.scss']
})
export class UserPageConfigurationComponent implements OnInit {

    queryInfo: QueryInfo;
    showSpinner: boolean = false;
    displayedColumns: string[] = [];
    dataSource: any;
    filter: UserPageConfigurationFilter;
    pageEvent: PageEvent;


    constructor(private userPageConfigurationService:  UserPageConfigurationService,
    ...
    ngOnInit() {
        ...
        this.filter = new  UserPageConfigurationFilter();
    ...
    getData(isExport: boolean) {
        ...
        if (isExport) {
            ...
            columnInfo = this.getDataConf().filter(x => x.isExport == true)
                .map((x) => ({
                    prop: x.prop,
                    localText: this.translate.instant('USERPAGECONFIGURATION.' + x.translate)
                }));

        }


        this.userPageConfigurationService.getData(this.filter, this.queryInfo, columnInfo, isExport)
    ...
    newItemDialog() {
        ...
        let emptyData = new UserPageConfigurationData();

        emptyData.userPageConfId = 0;
emptyData.userId = 0;
emptyData.confData = '';
emptyData.confPage = '';
...
emptyData.updatedUser = 0;
    ...
    editItemDialog(row: UserPageConfigurationData) {
        this.openDialog(row, false);
    }

    openDialog(row: UserPageConfigurationData, isNew: boolean) {

        this.dialog.open(UserPageConfigurationDialogComponent, {
            width: '100%',
            hasBackdrop: true,
            disableClose:true,
            data: {
    ...
    delete(data: UserPageConfigurationData) {
        ...
        this.dialog.open(ComfirmDialogComponent, {
            ...
            .subscribe(response => {
                if (response.comfirm == true) {
                    this.userPageConfigurationService.delete(deleteRow).subscribe((response: any) => {
                         this.navigationService.sessionControl(response);
                        if (response.isSuccess) {
...
export class UserPageConfigurationData {

    userPageConfId : number;
userId : number;
confData : string;
confPage : string;
...
export class UserPageConfigurationFilter {
