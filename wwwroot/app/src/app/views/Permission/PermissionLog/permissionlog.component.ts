...
import { PermissionLogDialogComponent } from './permissionlog-dialog.component';
...
@Component({
    selector: 'permissionlog-table',
    templateUrl: './permissionlog.component.html',
    styleUrls: ['./permissionlog.component.scss']
})
export class PermissionLogComponent implements OnInit {
    ...
    constructor(private permissionLogService: PermissionLogService,
    ...
    newItemDialog() {
        ...
        let emptyData = new PermissionLogData();

        emptyData.permissionLogId = 0;
    ...
    editItemDialog(row: PermissionLogData) {
        this.openDialog(row, false);
    }

    openDialog(row: PermissionLogData, isNew: boolean) {

        this.dialog.open(PermissionLogDialogComponent, {
            width: '100%',
            hasBackdrop: true,
            disableClose: true,
            data: {
        ...
        this.dialog.open(ComfirmDialogComponent, {
            ...
            .subscribe(response => {
                if (response.comfirm == true) {
                    this.permissionLogService.delete(deleteRow).subscribe((response: any) => {
                        this.navigationService.sessionControl(response);
                        if (response.isSuccess) {
...
export class PermissionLogData {

    permissionLogId: number;
    employeeId: number;
    employeeName: string;
    status: number;
    permissionId: number;
...
export class PermissionLogFilter {
